...
negbinom.res <- readRDS("res.sim.2M.1500.region.5000.negbinom.MAF.0.5.boot.keep.ends.rds") 
```
...
negbinom.pnt.geom <- lapply(negbinom.res, function(x) {x[[1]][[1]]}) %>% unlist()
...
negbinom.pnt.geom2 <- lapply(negbinom.res, function(x) {x[[2]][[1]]}) %>% unlist()
```
...
negbinom.lik.geom <- lapply(negbinom.res, function(x) {x[[1]][[2]]}) %>% unlist()
...
negbinom.lik.geom2 <- lapply(negbinom.res, function(x) {x[[2]][[2]]}) %>% unlist()
...
negbinom.pnt.AIC <- ifelse(negbinom.lik.geom < negbinom.lik.geom2, 1/negbinom.pnt.geom, 2/negbinom.pnt.geom2)
```
...
negbinom.boot <- lapply(negbinom.res, function(x) {x[[5]] %>% unlist() %>% matrix(byrow = TRUE, ncol = 2) %>% apply(2, function(x) quantile(x, probs = c(0.025, 0.975)))}) 
```
...
negbinom.coverage <- lapply(negbinom.boot, function(x) {c(x[1,1] <= 300 & 300 <= x[2,1], x[1,2] <= 300 & 300 <= x[2,2])}) %>% unlist() %>% matrix(byrow = TRUE, ncol = 2)
```
