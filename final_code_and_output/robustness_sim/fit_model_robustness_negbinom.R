...
tracts_negbinom <- read.csv("sim_tracts_vcf_negbinom_multiple_iterations.csv")
colnames(tracts_negbinom) <- c("V1", "V2", "length", "iter")
tracts_negbinom <- dplyr::filter(tracts_negbinom, iter <= 19)
tracts_negbinom <- dplyr::select(tracts_negbinom, -length)
tracts_negbinom_df_list <- split(tracts_negbinom, tracts_negbinom$iter)
res_negbinom <- lapply(tracts_negbinom_df_list, fit_model_M, MAF.chrom.1, 1500, 5000, 0.5)

saveRDS(res_negbinom, "res.sim.2M.1500.region.5000.negbinom.MAF.0.5.boot.keep.ends.rds")
