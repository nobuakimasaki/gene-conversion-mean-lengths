...
boot_MLE_M <- function(x, l_lst, psi_lst, M, remove.singletons) {
  ...
  if (remove.singletons) { 
    optim.out.geom <- optim(0.005, calc_neg_log_lik, psi_lst = psi_lst_boot, pL = pL_geom2_1M,
                            l_lst = l_lst_boot, M = M, lower = 0.0001, upper = 0.05, method = "Brent")}
  else { 
    optim.out.geom <- optim(0.005, calc_neg_log_lik, psi_lst = psi_lst_boot, pL = pL_geom2_M,
                            l_lst = l_lst_boot, M = M, lower = 0.0001, upper = 0.05, method = "Brent")}
...
fit_model_M <- function(l_lst, psi_lst, M, remove.singletons) {
  ...
  ### maximum likelihood
  if (remove.singletons) {
    optim.out.geom <- optim(0.005, calc_neg_log_lik, psi_lst = psi_lst, pL = pL_geom2_1M,
                            l_lst = l_lst, M = M, lower = 0.0001, upper = 0.05, method = "Brent")
  }
  else {
    optim.out.geom <- optim(0.005, calc_neg_log_lik, psi_lst = psi_lst, pL = pL_geom2_M,
                            l_lst = l_lst, M = M, lower = 0.0001, upper = 0.05, method = "Brent")
  }
...
file_name <- paste0("res.UK_Biobank.1M.1500.region.5000.geom2.ibdclust2cM.MAF.0.5.boot.grouped.keep.ends.rds")
