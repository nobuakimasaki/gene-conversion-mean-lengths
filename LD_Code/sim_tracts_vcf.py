...
#     print(i)
#     # Randomly sample one individual and obtain genotype data
#     idx = np.random.choice(genotype_calls.shape[1])
#     genotypes = genotype_calls[:, idx]
#     # Get one gene conversion tract
#     actual = gene_conversion_tracts[i]
#     L.append(sim_gene_conv(genotypes, actual, variant_positions, exc))
#     end_time = time.time()
...
# # Randomly sample N individuals (columns) from the genotype calls
# sampled_individuals_indices = np.random.choice(genotype_calls.shape[1], N, replace=True)
# # Initialize a list to store genotype calls for sampled individuals
# sampled_genotypes = []
# # Iterate over sampled individual indices and extract their genotype calls
# for idx in sampled_individuals_indices:
#     # Extract genotype calls for the sampled individual
#     individual_genotypes = genotype_calls[:, idx]
#     # Convert genotype calls to list and append to the sampled_genotypes list
#     sampled_genotypes.append(individual_genotypes.tolist())
#
# gene_conversion_tracts = sim_tracts(N, 1/300, min(variant_positions), max(variant_positions), "geom")
#
# # Using a lambda function
# res = list(map(lambda x, y: sim_gene_conv(x, y, variant_positions, exc), sampled_genotypes, gene_conversion_tracts))
# print(res)
